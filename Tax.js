     ...
     let tax = 0;
      for (const rate of taxRates) {
        if (basicSalary > rate.min) {
         ...
         if (basicSalary > rate.min) {
             const taxableAmount = Math.min(basicSalargy, rate.max) - rate.min; 
             tax += taxableAmount * rate.rate;
             } 
      }
            // Other deductions and calculations const nhif =
             calculateNHIF(basicSalary); 
                    ...
                    const nhifRates = [ 
                        { min: 0, max: 5999, deduction: 150 },
                         ...
                         const applicableRate = 
                          nhifRates.find((rate) => grossPay >= rate.min && grossPay <= rate.max); 
                        ...
                        // Helper function to calculate NSSF deduction
                         function calculateNSSF(grossPay) { 
                            // NSSF rates
                             ...
                             // Calculate NSSF deduction return grossPay * nssfRate; 
                             } 
                             // Example usage
